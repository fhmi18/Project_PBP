...
public class FavoriteFragment extends Fragment {
    ...
    private List<MenuItem> masterFavoriteList = new ArrayList<>();
    ...
    private void fetchFavorites() {
        Query favoriteQuery = db.collection("users").document(currentUser.getUid())
                .collection("favorites")
                .orderBy("addedAt", sortDirection);

        favoriteQuery.get().addOnSuccessListener(queryDocumentSnapshots -> {
            if (queryDocumentSnapshots.isEmpty()) {
                masterFavoriteList.clear();
                applyFilters();
                return;
            }

            List<Task<DocumentSnapshot>> menuTasks = new ArrayList<>();
            Map<String, DocumentSnapshot> favoriteDocs = new HashMap<>();

            for (QueryDocumentSnapshot favoriteDoc : queryDocumentSnapshots) {
                String menuId = favoriteDoc.getString("menuId");
                String campusId = favoriteDoc.getString("campusId");
                String canteenId = favoriteDoc.getString("canteenId");
                String categoryPath = favoriteDoc.getString("categoryPath");
                ...
                favoriteDocs.put(menuId, favoriteDoc);
            }
            ...
            Tasks.whenAllSuccess(menuTasks).addOnSuccessListener(results -> {
                ...
                for (Object result : results) {
                    ...
                    if (menuDoc.exists()) {
                        ...
                        if (menuItem != null) {
                            DocumentSnapshot favoriteDoc = favoriteDocs.get(menuDoc.getId());
                            menuItem.setMenuId(menuDoc.getId());
                            menuItem.setCampusId(favoriteDoc.getString("campusId"));
                            menuItem.setCanteenId(favoriteDoc.getString("canteenId"));
                            menuItem.setCategoryPath(favoriteDoc.getString("categoryPath"));
                            ...
                            masterFavoriteList.add(menuItem);
                        }
                    }
                }
                applyFilters();
    ...
    private void applyFilters() {
        if (activeCategoryFilters.isEmpty()) {
            adapter.updateData(masterFavoriteList);
            return;
        }

        List<MenuItem> filteredList = masterFavoriteList.stream()
